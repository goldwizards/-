...
// 에너지포(야마토포) 전용 사운드: 충전은 단계별로 더 선명, 발사는 '빔' 체감 강화
  ...
  function runSelfCheck(){
    ...
    setMsg(ok ? "자가진단: OK" : `자가진단: ${miss.length}개 문제 (버그리포트 복사로 공유)`, ok ? 1.6 : 3.0);
    return { ok, miss };
  }
...
{ id:"resource_tax", name:"보급 차감", desc:"이번 웨이브 동안 크리스탈 보상 -40%.",
  ...
  const DIFF_PRESETS = {
    ...
    normal: { key:"normal", name:"보통",   hpMul:1.00, dmgMul:1.00, spdMul:1.00, spawnMul:1.00, rewardMul:1.00,
              ...
              bossHpMul:1.00, bossSpdMul:1.00, bossPatternMul:1.00, bossRewardMul:1.00,
              finalHpMul:1.00, finalSpdMul:1.00, finalPatternMul:1.00, finalPhase2Add:0.00, finalRewardMul:1.00 },
    hard:   { key:"hard",   name:"어려움", hpMul:1.20, dmgMul:1.00, spdMul:1.06, spawnMul:1.00, rewardMul:1.05,
              eliteAdd:0.02,
  ...
  function setDiffPreset(key){
    ...
    if (state && state.diffLocked) {
      try { if (typeof setMsg === 'function') setMsg("진행 중에는 난이도 변경이 불가능합니다. (재시작 후 변경 가능)", 2.2); } catch {}
...
state.core.rebuildEmergencyUntil = t + dur;
  ...
  const UPGRADE_DEFS = [
    ...
    { id:"turretCrit", cat:"turret", name:"치명타", max:5, base:75, grow:1.60,
      desc:(lv)=>{
        ...
        return isHard
          ? `치명타 확률 ${total}% (x${mult.toFixed(1)})`
          : `치명타 확률 +${per*lv}% (x${mult.toFixed(1)})`;
      },
      apply(){} },
  ...
  for (const [cat, title] of CAT_META) {
    ...
    `;
	    ...
	    if (wantShopItems) {
	      ...
	      let lblRepair = `즉시 수리 (-${SHOP_ITEM_REPAIR_COST})`;
	      if (hpFull) lblRepair = "즉시 수리 (HP 만땅)";
	      else if (blockLeft > 0.05) lblRepair = `즉시 수리 (차단 ${blockLeft.toFixed(1)}s)`;
	      else if (cdR > 0.05) lblRepair = `즉시 수리 (${cdR.toFixed(1)}s)`;

	      let lblFire = `공속 10s (-${SHOP_ITEM_FIRE_COST})`;
	      if (!inWave) lblFire = `공속 10s (-${SHOP_ITEM_FIRE_COST}) · 웨이브 중`;
	      else if (cdF > 0.05) lblFire = `공속 10s (${cdF.toFixed(1)}s)`;
	      if (fireLeft > 0.05) lblFire += ` · 활성 ${fireLeft.toFixed(1)}s`;

	      let lblSlow = `웨이브 슬로우 (-${SHOP_ITEM_SLOW_COST})`;
	      if (!inWave) lblSlow = `웨이브 슬로우 (-${SHOP_ITEM_SLOW_COST}) · 웨이브 중`;
	      else if (slowAlready) lblSlow = "웨이브 슬로우 (적용중)";
	      else if (cdS > 0.05) lblSlow = `웨이브 슬로우 (${cdS.toFixed(1)}s)`;

	      const hint = "소비 아이템: 즉시 수리 / 공속 10s / 슬로우(1 웨이브)";
	      out += `
	        <div class="${rowClass}" style="cursor:default;" title="${hint}">
	          <div class="upgTop">
	            <div class="upgNameLine">
	              <b>소비 아이템</b>
	              <span class="muted">유틸 탭 전용</span>
	            </div>
	          ...
	          <div class="upgDesc">
	            <div><span class="muted">즉시 수리</span> 부족분의 35% (최소 20) 회복</div>
	            <div><span class="muted">공속/슬로우</span> 웨이브 중 사용 · 공속 ×${SHOP_ITEM_FIRE_MUL.toFixed(2)} · 적 이동 ×${SHOP_ITEM_SLOW_MUL.toFixed(2)}</div>
	          </div>
	        </div>
	      `;
	    }
    ...
    if (cat === "turret") {
      ...
      const label = (cur === "pierce") ? "A 관통" : (cur === "focus" ? "B 집중" : "미선택");
      const hint = unlocked ? (canPick ? "A/B 중 하나를 선택할 수 있습니다." : "웨이브/결과 화면에서는 변경할 수 없습니다.") : `잠김: 포탑 화력 Lv ${needLv} 필요`;
      ...
      out += `
        <div class="upgRow ${unlocked ? "canBuy" : "disabled"}" style="cursor:default;" title="${hint}">
          <div class="upgTop">
            <div class="upgNameLine">
              <b>기본 포탑 진화</b>
              <span class="muted">${unlocked ? `현재: ${label}` : `Lv ${needLv} 필요`}</span>
            </div>
            <div style="display:flex; gap:6px;">
              <button class="miniBtn" data-basic-evo="pierce" ${canPick ? "" : "disabled"} ${selStyle(cur==="pierce")}>A 관통</button>
              <button class="miniBtn" data-basic-evo="focus"  ${canPick ? "" : "disabled"} ${selStyle(cur==="focus")}>B 집중</button>
            </div>
          </div>
          <div class="upgDesc">
            <div><span class="muted">A</span> 관통 2회 · 피해 -12% · 탄속 +10% · 사거리 +5%</div>
            <div><span class="muted">B</span> 피해 +55% · 공속 -30% · 보스 피해 +35% · 사거리 +8%</div>
          </div>
        </div>
      `;
    }
...
function bindUpgradeControls(){
  ...
  for (const list of upgContainers){
    ...
    list.addEventListener("click", (ev)=>{
	  ...
	  // 업그레이드 버그(호버/툴팁이 리렌더로 끊기는 문제) 방지:
	  // 리스트에 마우스를 올린 동안에는 주기적 리렌더를 잠시 멈춘다.
	  ...
	  for (const list of upgContainers){
	    ...
	    // 일부 환경에서 pointerenter/leave가 불안정할 때를 대비
...
document.addEventListener("keydown", (ev)=>{
  ...
  // (UI) 상단 '미선택' 배지 삭제됨 — 아래 선택 요약 박스에 글로우 적용
...
function loadRecords(){
  try {
    ...
    if (!raw) {
      // (권장) legacy(v1) -> v2_normal 1회 마이그레이션
      try {
...
function basicEvoName(e){
  if (e === "pierce") return "A 관통";
  if (e === "focus") return "B 집중";
  return "미선택";
}

function mapName(mid){
  if (mid === "sanctuary") return "성역";
  if (mid === "void") return "공허";
  if (mid === "wasteland") return "황무지";
  return mid || "—";
}
...
function renderRecords(force=false){
  ...
  const lines = [];
  if (best) {
    lines.push(`<b>최고 웨이브</b>: WAVE ${best.wave} · ${formatTime(best.time||0)} · ${passiveName(best.passive)} · 맵 ${mapName(best.mapId)}`);
    lines.push(`<span class="small">포탑 비율: ${turretRatioText(best.turretBuilt)}</span>`);
    lines.push(`<span class="small">기본 포탑 진화: ${basicEvoName(best.basicEvo)}</span>`);
  } else {
  ...
  if (last) {
    ...
    lines.push(sep + `<b>마지막 런</b>: WAVE ${last.wave} · ${formatTime(last.time||0)} · ${passiveName(last.passive)} ${last.win ? "(승리)" : "(실패)"} · 맵 ${mapName(last.mapId)}`);
    lines.push(`<span class="small">포탑 비율: ${turretRatioText(last.turretBuilt)}</span>`);
    lines.push(`<span class="small">기본 포탑 진화: ${basicEvoName(last.basicEvo)}</span>`);
  }
...
function mainMenuRecordText(){
  ...
  const last = __records.lastRun;
  const lines = [];
  if (best) lines.push(`최고 기록: WAVE ${best.wave}`);
  else lines.push("최고 기록: 없음");
  if (last) lines.push(`직전 기록: WAVE ${last.wave} ${last.win ? "(승리)" : "(실패)"}`);
  else lines.push("직전 기록: 없음");
...
function updateMainMenuUI(force=false){
  ...
  if (mmNote) mmNote.textContent = `난이도: ${d.name} · (1웨이브 시작 후 변경 불가)`;
  if (mmRecords) {
...
if (btnMMStart) btnMMStart.onclick = ()=>{
  ...
  // 시작하자마자 바로 웨이브는 아니고, 기존대로 빌드 화면 유지
...
if (btnMMResetRec) btnMMResetRec.onclick = ()=>{
  ...
  try {
    ...
    if (typeof setMsg === "function") setMsg("런 기록이 초기화되었습니다.", 2.0);
  } catch(e){}
};
...
const CONTRACTS = {
  none:   { id:"none",   name:"선택 안함",       hpMul:1,    spdMul:1,    shieldMul:1,    grantShieldMul:0,    rewardMul:1.00, desc:"계약 없이 진행합니다." },
  hp:     { id:"hp",     name:"적 체력 강화",   hpMul:1.25, spdMul:1,    shieldMul:1,    grantShieldMul:0,    rewardMul:1.30, desc:"적 HP +25%" },
  spd:    { id:"spd",    name:"적 속도 강화",   hpMul:1,    spdMul:1.18, shieldMul:1,    grantShieldMul:0,    rewardMul:1.25, desc:"적 속도 +18%" },
  shield: { id:"shield", name:"적 보호막 강화", hpMul:1,    spdMul:1,    shieldMul:1.35, grantShieldMul:0.25, rewardMul:1.30, desc:"보호막 +35% (없는 적은 HP의 25%만큼 실드 부여)" },
};
...
function applyContractForWave(){
  ...
  if (pick !== "none") {
    try {
      fxText(`계약: ${c.name} (보상×${(c.rewardMul||1).toFixed(2)})`, CORE_POS.x, CORE_POS.y - 120, "#fbbf24");
...
function refreshContractUI(){
  ...
  if (uiContractMul) {
    ...
    uiContractMul.textContent = `×${mul.toFixed(2)}`;
  }
  ...
  if (uiContractHint) {
    if (isFinal) {
      uiContractHint.innerHTML = "최종 웨이브에는 위험 계약이 적용되지 않습니다.";
    } else if (state.phase === "wave") {
      uiContractHint.innerHTML = `이번 웨이브: <b>${cAct.name}</b> — ${cAct.desc} / 보상 <b>×${(cAct.rewardMul||1).toFixed(2)}</b>`;
    } else {
      uiContractHint.innerHTML = `선택: <b>${cPick.name}</b> — ${cPick.desc} / 보상 <b>×${(cPick.rewardMul||1).toFixed(2)}</b>`;
    }
  }
}
...
// 미션 목표는 "불가능한 목표"가 뜨지 않게, 웨이브/테마에 맞춰 목표치를 동적으로 계산합니다.
// 또한, 다음 웨이브의 목표를 미리 뽑아(build/clear에서) 표시하고, 웨이브 시작 시 확정되도록 합니다.

function buildEnemyPoolForWave(w, spec){
  // pickEnemyId()와 동일한 풀/가중치를 (확률 계산용으로) 복제
  const pool = [];
  pool.push(["grunt", 60]);
  ...
  // boss wave 보정은 여기서는 불필요(미션은 보스 웨이브에 안 뜸)이나,
  // 향후 변경에 대비해 동일하게 유지
  if (spec && spec.isBoss) {
    for (let i=0;i<pool.length;i++) pool[i][1] *= (pool[i][0]==="grunt" ? 0.55 : 1.25);
  }

  // theme wave 보정(확률 계산용)
...
const MISSION_DEFS = [
  {
    id:"runner", name:"질주병 사냥", kind:"kill_runner",
    canOffer:(w, spec)=>{
      if (!spec || spec.isBoss || spec.isFinal) return false;
      if ((w|0) < 3) return false;
      // 평균적으로 2마리 이상은 나올 웨이브에서만
      const exp = estimateExpectedCount(w, spec, "runner");
      return exp >= 2.2;
    },
    make:(w, spec)=>{
      const target = calcRunnerTarget(w, spec);
      return { target, desc:`이 웨이브에 질주병 ${target}마리 처치` };
    },
    reward:(w, m)=> Math.floor(18 + (w|0)*3 + ((m && m.target)|0)*3),
  },
  {
    id:"nodmg", name:"코어 무피해", kind:"no_hp_damage",
    canOffer:(w, spec)=>{
      if (!spec || spec.isBoss || spec.isFinal) return false;
      return (w|0) >= 3;
    },
    make:(w, spec)=>({ desc:"이 웨이브에 코어 HP 피해 없이 버티기" }),
    reward:(w)=> Math.floor(30 + (w|0)*5),
  },
  {
    id:"limit", name:"소수 정예", kind:"turret_limit",
    canOffer:(w, spec)=>{
      if (!spec || spec.isBoss || spec.isFinal) return false;
      return (w|0) >= 3;
    },
    make:(w, spec)=>{
      ...
      return { limit, desc:`이 웨이브 동안 포탑 ${limit}개 이하로 버티기` };
    },
    reward:(w, m)=>{
...
function finalizeMissionOnWaveClear(clearedWave){
  ...
  if (ok) {
    ...
    try {
      fxText(`목표 달성! +${bonus}`, CORE_POS.x, CORE_POS.y - 92, "#a7f3d0");
      fxRing(CORE_POS.x, CORE_POS.y, CORE_RADIUS+10, CORE_RADIUS+150, "#a7f3d0");
    } catch(e) {}
  } else {
    try {
      fxText("목표 실패", CORE_POS.x, CORE_POS.y - 92, "#fca5a5");
    } catch(e) {}
  }
}
...
function missionHtml(m, inWave, label){
  ...
  line += `<b>${m.name}</b> — ${m.desc}`;

  if (m.kind === "kill_runner") {
    if (inWave) line += `<br>진행: <b>${m.progress||0}/${m.target||0}</b>`;
    else line += `<br>목표: <b>${m.target||0}</b>마리`;
  } else if (m.kind === "turret_limit") {
    const cur = (state.turrets ? state.turrets.length : 0);
    if (inWave) line += `<br>현재/최대 포탑: <b>${cur}/${m.maxTurrets||0}</b> (제한 ${m.limit})`;
    else line += `<br>현재 포탑: <b>${cur}</b> (제한 ${m.limit})`;
  } else if (m.kind === "no_hp_damage") {
    if (inWave) line += `<br>상태: <b>${m.failed ? "실패" : "유지 중"}</b>`;
    else line += `<br>조건: <b>코어 HP 피해 0</b>`;
  }

  if (m.done) {
    line += `<br><span style="color:${m.succeeded ? '#a7f3d0' : '#fca5a5'}; font-weight:800">${m.succeeded ? '성공' : '실패'}</span>`;
  } else if (m.failed) {
    line += `<br><span style="color:#fca5a5; font-weight:800">실패</span>`;
  }
...
function refreshMissionUI(){
  ...
  // reward badge: 전투 중에는 현재 미션, 그 외에는 다음 미션(있으면)
  if (uiMissionReward) {
    let shown = 0;
    let op = 0.55;
    if (inWave) {
  ...
  if (inWave) {
    if (!m) {
      uiMission.innerHTML = "목표: 없음";
      return;
    }
    uiMission.innerHTML = missionHtml(m, true, "목표:");
    return;
  }

  // build/clear/finalprep: 다음 미션을 먼저 보여주고, 직전 미션 결과는 아래에 작게 표시
  let out = "";
  if (nm) {
    out = missionHtml(nm, false, "다음 목표:");
  } else {
    out = "다음 웨이브 목표: 없음";
  }

  if (m && m.done) {
    const tag = m.succeeded ? "성공" : "실패";
    out += `<div style="margin-top:6px; opacity:0.70">지난 목표: <b>${tag}</b> — ${m.name}</div>`;
  }
...
// ===== End Wave Missions =====
  ...
  function corePassiveDescLines(pid){
    ...
    if (pid === 'rebuild'){
      return [
        `저체력(HP 70%↓)부터 피해감소가 붙습니다. (10%에서 최대 -${_fmtPct(12*m)}%)`,
        `실드가 깨지면 짧은 시간 ‘긴급 보강’(피해 -${_fmtPct(38*m)}%)이 발동합니다. (쿨 7초)`,
        `보호막 재생 +${_fmtPct(15*m)}% (최종전 추가 +${_fmtPct(10*m)}%)`,
        `저체력일수록 방어/보호막방어가 증가합니다. (최대 방어 +${_fmtFixed(15*m,1)} / 보호막방어 +${_fmtFixed(7.5*m,1)})`,
        `HP 자동 수리가 강화됩니다. (저체력일수록 회복량↑ / 최종전 딜레이↓)`
      ];
    }
    ...
    if (pid === 'resonance'){
      return [
        `보호막이 흡수한 피해로 ‘공명 게이지’가 차오릅니다.`,
        `게이지에 따라 포탑 피해/공속이 증가합니다. (최대 피해 +${_fmtPct(30*m)}%, 공속 +${_fmtPct(18*m)}%)`,
        `게이지 100%가 되면 최근 흡수량 기반의 ‘공명 방출’이 자동 발동합니다. (방출 피해 계수 ${_fmtPct(105*m)}%)`,
        `방출 시 주변 확산 피해 + ‘노출’ 디버프가 걸립니다. (노출: 일반 +${_fmtPct(24*m)}% / 보스 +${_fmtPct(13*m)}%)`,
        `피격 후 즉시 반격하는 타입이 아니라, 누적→방출형 패시브입니다.`
      ];
    }

    if (pid === 'overload'){
      // OVERLOAD_CFG 값은 실행 시점에 이미 정의되어 있음
      ...
      return [
        `HP 30%↓ 진입 시(내려올 때) 쇼크웨이브(넉백+둔화 ${_fmtSec(slowDur)}s) + 과부하 버스트 ${_fmtSec(burstDur)}s 발동 (쿨 18s)`,
        `포탑 적중 시 표식(최대 5중첩/${_fmtSec(markDur)}s 갱신): 일반/엘리트 +${_fmtPct(markN)}%p×중첩, 보스/최종보스 +${_fmtPct(markB)}%p×중첩`,
        `버스트 ${_fmtSec(burstDur)}초: 포탑 탄 관통 +1 / (스플래시 없는 포탑은) ${Math.round(miniR)}px 소형 폭발(${_fmtPct(miniMul)}%) 추가`,
        `버스트 ${_fmtSec(burstDur)}초: 최종보스 ‘포탑 내성’ ${_fmtPct(ignore)}% 부분 무시 + 투사체 붉은 트레일`,
        `연계: HP≤40% 수리 시 주변 적(보스 우선) 표식 +2 / 버스트 남은 <2s면 +${_fmtSec(extend)}s 연장(20s ICD) / 긴급 보호막 사용 시 다음 쿨 -${_fmtSec(cdReduce)}s(20s ICD)`
      ];
    }
    ...
    if (pid === 'overdrive'){
      return [
        ...
        `저체력 구간에서 보호막 재생이 증가합니다.`,
        `저체력일수록 받는 피해가 소폭 감소합니다.`,
        `에너지포가 광역 피해(${_fmtPct(30*m)}%)를 추가로 입힙니다.`
      ];
    }
...
// ===== Canvas Passive Pick (in-game) =====
// 요청 UI(스케치): 상단=선택/옵션, 하단=효과 설명(긴 글은 이쪽으로)
const CORE_PASSIVE_TAG = {
  rebuild:   "안정 / 장기전",
  resonance: "공격 / 포탑 중심",
  overload:  "저체력 폭발 / 표식",
  overdrive: "코어 직접 공격"
...
function passivePickLayout(){
  const panelW = clamp(Math.round(W * 0.78), 520, 880);
  const panelH = clamp(Math.round(H * 0.54), 360, 560);
  const x = Math.round(W/2 - panelW/2);

  // 중앙보다 살짝 위
  let y = Math.round(H * 0.16);
  y = clamp(y, 56, H - panelH - 64);

  const pad = clamp(Math.round(panelW * 0.028), 14, 20);
  const gap = clamp(Math.round(panelW * 0.02), 10, 16);

  const headerH = 84;                 // 타이틀 + 안내
  const topH = clamp(Math.round(panelH * 0.36), 140, 200); // 옵션/선택 영역
  const splitY = y + headerH + topH;

  // 옵션 버튼(2x2)
  const gridX = x + pad;
  const gridY = y + headerH + 10;
  const gridW = panelW - pad*2;
  const btnW = Math.floor((gridW - gap) / 2);
  const btnH = clamp(Math.round(topH * 0.44), 56, 74);

  const opts = [
    {id:'rebuild',   x:gridX,          y:gridY,          w:btnW, h:btnH},
    {id:'resonance', x:gridX+btnW+gap, y:gridY,          w:btnW, h:btnH},
    {id:'overload',  x:gridX,          y:gridY+btnH+gap, w:btnW, h:btnH},
    {id:'overdrive', x:gridX+btnW+gap, y:gridY+btnH+gap, w:btnW, h:btnH},
  ];

  // 적용 버튼(우하단)
  const confirmW = clamp(Math.round(panelW * 0.22), 150, 210);
  const confirmH = 46;
  const confirmX = x + panelW - pad - confirmW;
  const confirmY = y + panelH - pad - confirmH;

  // 설명 박스(하단) + 하단 버튼 공간 확보
  const footerH = 58;
  const descY = splitY + 16;
  const descH = Math.max(120, (y + panelH - descY - pad - footerH));
  const desc = { x:x+pad, y:descY, w:panelW-pad*2, h:descH };

  return {
    panel:{x,y,w:panelW,h:panelH,r:18},
    header:{h:headerH},
    top:{y:y+headerH, h:topH, splitY},
    options:opts,
...
function ptInRect(mx, my, r){
  return (mx >= r.x && mx <= r.x+r.w && my >= r.y && my <= r.y+r.h);
}

function tryPickPassiveAt(mx, my){
  // 코어 패시브는 \"미선택\" 상태에서만 (재시작 시 다시 선택)
  if (state.core.passiveId) return false;
  if (!(state.phase === 'build' || state.phase === 'clear' || state.phase === 'finalprep')) return false;

  const L = passivePickLayout();

  // 옵션 클릭: 상단에서 선택 대상(미리보기)만 지정
  for (const r of L.options){
    if (ptInRect(mx, my, r)){
      state.core.passivePreviewId = r.id;
      try { SFX.play("click"); } catch {}
  ...
  // 적용 버튼: 미리보기 확정 후 패널 닫기
  if (ptInRect(mx, my, L.confirm)) {
	  const id = state.core.passivePreviewId;
	  if (!id) {
	    try { SFX.play("click"); } catch {}
	    try { setMsg("코어 패시브를 먼저 선택하세요.", 2.0); } catch {}
	    return true;
	  }
	  selectCorePassive(id);
	  try { refreshCorePassiveUI(); } catch {}
	  try { setMsg(`코어 패시브 적용: ${CORE_PASSIVES[id].name}`, 1.6); } catch {}
	  return true;
  }
...
function wrapLines(ctx, text, maxW){
  ...
  for (const part of parts){
    const cand = line ? (raw.includes(" ") ? (line + " " + part) : (line + part)) : part;
...
function drawPassivePickOverlay(){
  if (state.core.passiveId) return;
  if (!(state.phase === 'build' || state.phase === 'clear' || state.phase === 'finalprep')) return;
  // 기본 미리보기 제거: 처음엔 미선택

  const L = passivePickLayout();
  const P = L.panel;
  const pick = state.core.passivePreviewId || null;
  const dPick = pick ? CORE_PASSIVES[pick] : null;
  ...
  ctx.fillText('코어 패시브 선택 (4개 중 1개)', W/2, P.y + 28);

  ctx.font = '650 13.5px system-ui, -apple-system, Segoe UI, Roboto, Arial';
  ctx.fillStyle = 'rgba(229,238,252,0.80)';
  ctx.fillText('위에서 1개를 선택 → 아래에서 효과 확인 → 오른쪽 아래 [적용]으로 확정', W/2, P.y + 56);
  ...
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.moveTo(P.x + 16, L.top.splitY);
  ctx.lineTo(P.x + P.w - 16, L.top.splitY);
  ctx.stroke();
  ctx.restore();

  // ---- Top area: options (모듈/선택) ----
  for (const r of L.options){
    const d = CORE_PASSIVES[r.id];
    const col = passiveAccent(r.id);
    const active = (r.id === pick);

    ctx.save();
    ctx.globalAlpha = 0.98;
    ctx.fillStyle = active ? 'rgba(16,28,48,0.96)' : 'rgba(11,19,32,0.88)';
    ctx.strokeStyle = active ? col : 'rgba(255,255,255,0.14)';
    ...
    ctx.fillText(d.name, r.x + 12, r.y + 10);

    ctx.font = '700 12.5px system-ui, -apple-system, Segoe UI, Roboto, Arial';
    ctx.fillStyle = 'rgba(229,238,252,0.78)';
    ctx.fillText(CORE_PASSIVE_TAG[r.id] || '', r.x + 12, r.y + 34);
  ...
  ctx.strokeStyle = 'rgba(255,255,255,0.10)';
  ctx.lineWidth = 1.6;
  roundRect(ctx, desc.x, desc.y, desc.w, desc.h, 14);
  ctx.fill(); ctx.stroke();

  // header
  ctx.textAlign = 'left';
  ctx.textBaseline = 'top';
  ctx.fillStyle = 'rgba(229,238,252,0.88)';
  ctx.font = '900 14px system-ui, -apple-system, Segoe UI, Roboto, Arial';
  const _hardTxt = (state && state.diffId === "hard") ? " (HARD: 80%)" : "";
  ctx.fillText('패시브 효과' + _hardTxt, desc.x + 12, desc.y + 10);

  // body
  const bodyX = desc.x + 12;
  let y = desc.y + 32;
  const maxW = desc.w - 24;
  const lineH = 16;

  ctx.font = '650 12.5px system-ui, -apple-system, Segoe UI, Roboto, Arial';
  ctx.fillStyle = 'rgba(229,238,252,0.84)';

  const lines = dPick ? corePassiveDescLines(dPick.id) : []; 
  if (!lines.length){
    ctx.fillText('옵션을 선택해 효과를 확인하십시오.', bodyX, y);
  } else {
    for (const item of lines){
      const bullet = '• ' + item;
      const wrapped = wrapLines(ctx, bullet, maxW);
      for (const wl of wrapped){
        if (y > desc.y + desc.h - 18) break;
        ctx.fillText(wl, bodyX, y);
        y += lineH;
      }
      y += 4;
      if (y > desc.y + desc.h - 18) break;
  ...
  ctx.restore(); // desc box

  // ---- Apply button (bottom-right) ----
  const C = L.confirm;
  ctx.save();
  ctx.globalAlpha = 0.98;
  ctx.fillStyle = 'rgba(16,28,48,0.96)';
  ctx.strokeStyle = pick ? passiveAccent(pick) : 'rgba(255,255,255,0.14)';
  ctx.lineWidth = 2.6;
  ...
  ctx.fillStyle = '#e5eefc';
  ctx.font = '900 16px system-ui, -apple-system, Segoe UI, Roboto, Arial';
  ctx.fillText('적용', C.x + C.w/2, C.y + C.h/2);
  ctx.restore();
...
function roundRect(ctx, x, y, w, h, r){
  ...
  function pulseCorePassiveBox(){
    ...
    void uiCorePassiveChosenWrap.offsetWidth; // reflow to restart animation
  ...
  function refreshCorePassiveUI(){
    ...
    // (UI) 상단 배지 대신: 선택/확정 시 아래 박스에 글로우 펄스
    if (uiCorePassiveChosenWrap) {
      ...
      // 확정/미리보기 상태이면 예전처럼 은은하게 계속 빛나는 효과(지속)
      ...
      // 2) 적용(확정) 순간에도 반드시 펄스 (미리보기와 동일 id여도)
    ...
    if (uiCorePassiveDesc) {
      if (!showId) {
        ...
        const hardInfo = (state && state.diffId === "hard")
          ? `<div class=\"muted\" style=\"margin-top:6px;\">HARD: 패시브 모든 효과 ×0.8 적용</div>`
          : "";
        uiCorePassiveDesc.innerHTML =
          lines.map(s=>`• ${s}`).join("<br>") +
          hardInfo +
  ...
  const corePassivePanelClick = (pid) => {
    ...
    // (선택 확정은 게임 화면 중앙 패널 우하단 [적용] 버튼으로)
...
// 업그레이드 입력: 모바일 스크롤 중 오구매 방지
// - pointerdown에서는 후보만 저장
// - 일정 거리 이상 움직이면(스크롤) 구매 취소
// - pointerup/click에서만 구매 실행
if (upgContainers.length) {
  let lastUpgHandledAt = 0;
  const tap = { active:false, moved:false, id:null, x0:0, y0:0, pid:-1, t0:0 };
  const MOVE_THR = 10;

  const tryBuyUpgId = (id, ev) => {
    const now = performance.now();
    if ((now - lastUpgHandledAt) < 220) return;
    ...
    if (!canBuy) {
      ...
      const msg =
        ...
        "지금은 강화할 수 없습니다!";
      try{ fxText(msg, CORE_POS.x, CORE_POS.y - 92, "#ff9fb2"); }catch{}
  ...
  const onDown = (ev) => {
    ...
    tap.active = true;
    tap.moved = false;
    tap.id = row.dataset.upg;
    tap.x0 = ev.clientX;
    tap.y0 = ev.clientY;
    tap.pid = ev.pointerId;
    tap.t0 = performance.now();
  };

  const onMove = (ev) => {
    if (!tap.active) return;
    if (tap.pid !== -1 && ev.pointerId !== tap.pid) return;
    const dx = ev.clientX - tap.x0;
    const dy = ev.clientY - tap.y0;
    if ((dx*dx + dy*dy) > (MOVE_THR*MOVE_THR)) tap.moved = true;
  };
  ...
  const onUp = (ev) => {
    ...
    tap.pid = -1;
    tap.id = null;
    if (moved) return; // 스크롤/드래그 -> 구매 방지
    if (!id) return;
    tryBuyUpgId(id, ev);
  };

  const onCancel = () => { tap.active=false; tap.pid=-1; tap.id=null; };
  ...
  for (const el of upgContainers) {
    ...
    el.addEventListener("click", (ev)=>{
      ...
      if (!row) return;
      if (tap.moved) return;
      tryBuyUpgId(row.dataset.upg, ev);
    }, { capture:true });
  }
...
if (upgContainers.length) {
  for (const el of upgContainers) {
    if (!el || el.__evoBound) continue;
    el.__evoBound = true;
    el.addEventListener("click", (ev)=>{
      const btn = ev.target.closest("[data-basic-evo]");
      if (!btn) return;
      ev.preventDefault(); ev.stopPropagation();
      ensureAudio();

      const kind = btn.dataset.basicEvo;
      const needLv = 3;
      if ((state.upg.turretDmg|0) < needLv) {
        try{ SFX.play("click"); }catch{}
        setMsg(`잠김: 포탑 화력 Lv ${needLv} 필요`, 1.8);
        return;
      }
      if (state.phase === "wave" || state.phase === "fail" || state.phase === "win") {
        try{ SFX.play("click"); }catch{}
        setMsg("지금은 변경할 수 없습니다.", 1.6);
        return;
      }
      if (kind !== "pierce" && kind !== "focus") return;

      state.basicEvo = kind;
      try{ SFX.play("click"); }catch{}
      try{ fxText(`진화 선택: ${kind === "pierce" ? "A 관통" : "B 집중"}`, CORE_POS.x, CORE_POS.y - 96, "#93c5fd"); }catch{}
      window.__upgLastRenderAt = 0;
      window.__upgForceRender = true;
      refreshUI();
    }, { capture:true });
...
// 소비 아이템(Utility 탭) 클릭 처리 — 데이터 속성 기반
document.addEventListener("click", (ev)=>{
  const btn = ev.target && ev.target.closest ? ev.target.closest("[data-shop-item]") : null;
  if (!btn) return;
  if (btn.tagName !== 'BUTTON') return;
  ev.preventDefault(); ev.stopPropagation();
  ensureAudio();
  const id = btn.dataset.shopItem;
  if (id === 'repair') useShopItemRepair();
  else if (id === 'fire') useShopItemFire();
  else if (id === 'slow') useShopItemSlow();
  window.__upgLastRenderAt = 0;
  window.__upgForceRender = true;
  refreshUI();
}, { capture:true });
...
const btnToggleWire = document.getElementById("btnToggleWire");
  ...
  const mbToggleBar = document.getElementById("mbToggleBar");
...
const cheatModal = document.getElementById("cheatModal");
  ...
  state.mobileBarCollapsed = true;
  state.mobileBarCollapseManual = false;
  ...
  function setMobileUIEnabled(on){
    ...
    if (on){
      ...
      state.mobileBarCollapseManual = false;
      state.mobileBarCollapsed = true;
      try{ applyMobileBarCollapsed(); }catch(e){}
...
// 모바일 버튼바 접기/펼치기 (화면 가림 완화)
function applyMobileBarCollapsed(){
  if (!mobileBar) return;
  const collapsed = !!state.mobileBarCollapsed;
  mobileBar.classList.toggle("collapsed", collapsed);
  if (mbToggleBar) mbToggleBar.textContent = collapsed ? "▲" : "▼";
  updateMobilePad();
}

function setMobileBarCollapsed(v, manual){
  state.mobileBarCollapsed = !!v;
  if (manual) state.mobileBarCollapseManual = true;
  applyMobileBarCollapsed();
}
...
// 자동 접기: 웨이브 중에는 가림을 줄이기 위해 접힘(사용자가 수동으로 토글하면 유지)
if (!state.mobileBarCollapseManual) {
  const wantCollapsed = !(state.phase==="build" || state.phase==="clear" || state.phase==="finalprep");
  if (!!state.mobileBarCollapsed !== !!wantCollapsed) {
    state.mobileBarCollapsed = !!wantCollapsed;
    applyMobileBarCollapsed();
  }
}
...
const s = state.selected;
  ...
  if (mbToggleBar) mbToggleBar.onclick = ()=> { try{ ensureAudio(); SFX.play("click"); }catch(e){} setMobileBarCollapsed(!state.mobileBarCollapsed, true); };
  ...
  canvas.addEventListener("pointerup", (e)=>{
    ...
    const {mx, my} = canvasXYFromClient(e.clientX, e.clientY);
    if (state.phase === "build" || state.phase === "clear" || state.phase === "finalprep"){
      // Passive pick has priority when not selected yet
      if (tryPickPassiveAt(mx, my)) { _down = null; e.preventDefault(); return; }
  ...
  if (btnBugReport) btnBugReport.onclick = () => {
    ...
    copyText(txt).then(()=>{
      setMsg("버그 리포트 복사됨 (클립보드)", 1.8);
    }).catch(()=>{
      try{
        window.prompt("버그 리포트(복사해서 보내세요):", txt);
        setMsg("복사 실패: 프롬프트로 출력했습니다.", 2.5);
      }catch(e){
        setMsg("복사 실패: 브라우저 권한을 확인하세요.", 2.5);
      }
    });
  };
...
state.mobileSellMode = false;
 ...
 SFX.play("click"); }
  ...
  canvas.addEventListener("mousedown", (e) => {
    ...
    // Passive pick (left click)
    if (e.button === 0 && tryPickPassiveAt(mx, my)) return;
...
function waveSpec(w){
  ...
  const ENEMY_ARCH = {
    ...
    supporter:{ name:"실드 서포터", hpMul:0.95, spdMul:0.92, r:12, reward:15, touchDmg:6, touchCd:0.92,
              shieldMul:0.55,
              supportCd:1.80, supportR:170, supportMul:0.18, supportMulBoss:0.10,
              coreOpts:{}, color:"#38bdf8" },

    disruptor:{ name:"교란기", hpMul:1.18, spdMul:0.90, r:12, reward:15, touchDmg:6, touchCd:0.95,
...
bomber:{ name:"폭파병",   hpMul:0.82, spdMul:1.25, r:12, reward:14, touchDmg:0, touchCd:0,
  ...
  const MAP_DEFS = {
    sanctuary: { id:"sanctuary", name:"성역", desc:"보호막 재생 +10%", shieldRegenMul:1.10, enemySpdMul:1.00, turretRangeMul:1.00, rewardMul:1.00 },
    void:      { id:"void",      name:"공허", desc:"EMP 이벤트 빈도↑ · 보상 +8%", shieldRegenMul:1.00, enemySpdMul:1.00, turretRangeMul:1.00, rewardMul:1.08, empBias:2 },
    wasteland: { id:"wasteland", name:"황무지", desc:"적 속도 +8% · 포탑 사거리 +8%", shieldRegenMul:1.00, enemySpdMul:1.08, turretRangeMul:1.08, rewardMul:1.04 },
  };
  ...
  function refreshMapUI(force=false){
    ...
    const m = mapById(id);
    state.map = m;
    if (uiMapBonus) uiMapBonus.textContent = `×${(m.rewardMul||1).toFixed(2)}`;
    uiMap.innerHTML = `<b>${m.name}</b> — ${m.desc}`;
  }
...
// (11) 방어선: 방벽(1초) — 코어 피해 무효
function tryBarrier(){
  if (state.phase === "fail" || state.phase === "win") return;

  // 방벽은 전투(웨이브) 중에만 사용
  if (state.phase !== "wave") {
    fxText("웨이브 중에만 사용할 수 있습니다", CORE_POS.x, CORE_POS.y - 70, "#93c5fd");
    try{ SFX.play("click"); }catch{}
    return;
  }
  ...
  if (state.core.barrierActive) {
    ...
    fxText(`방벽 ${left.toFixed(1)}s`, CORE_POS.x, CORE_POS.y - 70, "#93c5fd");
    try{ SFX.play("click"); }catch{}
    return;
  }
  if (cdLeft > 0.05) {
    fxText(`방벽 쿨다운 ${cdLeft.toFixed(1)}s`, CORE_POS.x, CORE_POS.y - 70, "#ffd166");
    try{ SFX.play("click"); }catch{}
    return;
  }
  ...
  fxText("방벽!", CORE_POS.x, CORE_POS.y - 64, "#93c5fd");
...
function useShopItemRepair(){
  ...
  if (cdLeft > 0) {
    fxText(`아이템 쿨다운 ${cdLeft.toFixed(1)}s`, CORE_POS.x, CORE_POS.y - 70, "#ffd166");
    try{ SFX.play("click"); }catch{}
    return;
  }
  if (state.core.hp >= state.core.hpMax - 0.01) {
  ...
  fxText(`즉시 수리 +${(heal|0)}`, CORE_POS.x, CORE_POS.y - 64, "#67f3a6");

  // 임계 과부하 연계(기존 수리 버튼과 동일)
  overloadOnRepair(hpFracBefore);
}
...
function useShopItemFire(){
  ...
  fxText(`공속 증가! ×${SHOP_ITEM_FIRE_MUL.toFixed(2)}`, CORE_POS.x, CORE_POS.y - 64, "#fcd34d");
  try{ SFX.play("click"); }catch{}
}

// (12) 상점 소비 아이템: 1 웨이브 슬로우
function useShopItemSlow(){
  ...
  if (state.shop.slowWave === state.wave) {
    fxText("이번 웨이브는 이미 슬로우 적용 중", CORE_POS.x, CORE_POS.y - 70, "#93c5fd");
    try{ SFX.play("click"); }catch{}
    return;
  }
  if (cdLeft > 0) {
  ...
  fxText(`슬로우 웨이브! ×${SHOP_ITEM_SLOW_MUL.toFixed(2)}`, CORE_POS.x, CORE_POS.y - 64, "#a7f3d0");
  try{ SFX.play("click"); }catch{}
}
...
// 웨이브가 오를수록 "위력(피해)"도 상승
    ...
    const dmgMul = (1 + Math.max(0, state.wave - 1) * 0.06) * (spec.isBoss ? 1.15 : 1.0);
        ...
        if (isFinalBossSpawn) {
          ...
          // (HP×2.05 / 피해×1.25 / 발사쿨×0.82 / 접촉×1.25 / 속도×1.10)
          // NOTE: Hard 모드에서만 HP 버프를 2.30으로 상향 적용.
...
function overloadShockBurst(){
  ...
  // 연출: 버스트 카드
  try { cineToast("임계 과부하", `버스트 ${Math.round(OVERLOAD_CFG.burstDur * passiveHardMul())}s`, "#fb7185", 1.05); } catch {}
  ...
  fxText(`과부하 버스트 ${Math.round(OVERLOAD_CFG.burstDur * passiveHardMul())}s`, CORE_POS.x, CORE_POS.y - 138, '#fda4af');
...
function overloadOnRepair(hpFracBefore){
  ...
  if (t >= (state.core.overloadExtendReadyAt||0) && overloadBurstActive()) {
    ...
    if (rem > 0 && rem < OVERLOAD_CFG.extendIfRemainLt) {
      ...
      fxText(`버스트 연장 +${(OVERLOAD_CFG.extendAdd * passiveHardMul()).toFixed(1)}s`, CORE_POS.x, CORE_POS.y - 104, '#fda4af');
    }
  }
}
...
function overloadOnAegis(){
  ...
  fxText(`과부하 쿨 -${(OVERLOAD_CFG.aegisCdReduce * passiveHardMul()).toFixed(1)}s`, CORE_POS.x, CORE_POS.y - 88, '#fb7185');
}
...
// 옵션: 특정 공격이 방어력을 일부 무시/강화
  ...
  // ✅ 첫 웨이브(1) 시작 순간 난이도 잠금: 재시작 전까지 변경 불가
...
if (state.phase !== "win") drawBossHUD();
    ...
    // In-game passive pick overlay (when not selected yet)
    if (state.phase !== 'win' && !state.core.passiveId) drawPassivePickOverlay();
...
function refreshUI(){
    ...
    if (state.core.passiveId === "rebuild") {
      const hpFrac = state.core.hpMax>0 ? (state.core.hp/state.core.hpMax) : 1;
      const tB = clamp((0.70 - hpFrac) / 0.60, 0, 1);
      // 하드모드에서는 패시브 모든 효과가 80% 적용되어야 하므로,
      // HUD 표기도 실제 패시브 스케일(passiveScale)과 동일하게 맞춘다.
      // (최대 방어 +15 / 보호막방어 +7.5)
    ...
    if (typeof hudStatusRow !== 'undefined' && hudStatusRow) {
      ...
      if (state.core.barrierActive){
        ...
        chips.push({ label:'방벽', left:left, color:'#93c5fd', pr:4 });
      }
      else if (barCd > 0.05) chips.push({ label:'방벽CD', left:barCd, color:'#93c5fd', pr:1 });

      const fireLeft = Math.max(0, (state.shop.fireUntil||0) - tNow);
      if (fireLeft > 0.05) chips.push({ label:'공속', left:fireLeft, color:'#fcd34d', pr:4 });
      if (state.phase==='wave' && state.shop.slowWave === state.wave) chips.push({ label:'슬로우', left:0, color:'#a7f3d0', pr:3 });
...
if (btnBarrier) {
  ...
  btnBarrier.textContent = barActive ? `방벽 (${barLeft.toFixed(1)}s)` : (barCdLeft>0.05 ? `방벽 (${barCdLeft.toFixed(1)}s)` : `방벽 (-${BARRIER_COST})`);
}

// Shop item 버튼 UI
if (uiShopItemHint) {
  uiShopItemHint.innerHTML = `즉시 수리(-${SHOP_ITEM_REPAIR_COST}) · 공속 10s(-${SHOP_ITEM_FIRE_COST}) · 슬로우 1웨이브(-${SHOP_ITEM_SLOW_COST})`;
}
if (btnItemRepair){
  ...
  btnItemRepair.textContent = cd>0.05 ? `즉시 수리 (${cd.toFixed(1)}s)` : `즉시 수리 (-${SHOP_ITEM_REPAIR_COST})`;
}
if (btnItemFire){
  ...
  btnItemFire.textContent = cd>0.05 ? `공속 10s (${cd.toFixed(1)}s)` : `공속 10s (-${SHOP_ITEM_FIRE_COST})`;
}
if (btnItemSlow){
  ...
  btnItemSlow.textContent = already ? `웨이브 슬로우 (적용중)` : (cd>0.05 ? `웨이브 슬로우 (${cd.toFixed(1)}s)` : `웨이브 슬로우 (-${SHOP_ITEM_SLOW_COST})`);
}
...
if (mbBarrier){
  ...
  const sp = mbBarrier.querySelector('span'); if (sp) sp.textContent = '장벽';
...
if (mbFinalRow) {
  ...
  function wireReset(){
    ...
    // 보호막 프레임(파랑)용 '실루엣 외곽' — 전체 와이어를 감싸는 1개 폴리곤
    const OUTLINE_NORM = [
      [-0.0133,-1.2433],
      [-0.5200,-0.6400],
      [-0.5267,-0.6267],
    ...
    // 보호막 테두리(파랑): 내부선이 아니라 '전체 실루엣' 1개만
    const outline = OUTLINE_NORM.map(([nx,ny])=>P(nx,ny));
  ...
  function drawWireStatusPanel(hpRatio, shieldRatio){
    ...
    if (shieldRatio > 0.001) {
      const lw = clamp(4 + 6*shieldRatio, 4, 12);
      wctx.save();
      wctx.globalAlpha = 0.75 + 0.20*shieldRatio;
      wctx.shadowColor = "rgba(0,0,0,0)";
      wctx.shadowBlur  = 0;
  ...
  function tick(){
    ...
    if (state.phase !== "wave") {
      ...
      // 상점(소비 아이템) 쿨/지속시간도 웨이브 시간만 소모되도록 정지
      const sh = state.shop;
      if (sh) {
  ...
  // === Mobile Bottom Tabs UI (A: 45vh) ===
  (function setupBottomTabsUI(){
    try{
      const isMobile = (window.matchMedia && window.matchMedia("(max-width: 900px)").matches) || (navigator.maxTouchPoints||0) > 0;
      if(!isMobile) return;
      ...
      if(!tabs || !sheet || !sheetCtrl || !sheetStatus || !sheetShop) return;

      document.body.classList.add("uiTabs");

      // helpers
      const qs = (sel, root=document) => root.querySelector(sel);
      const qsa = (sel, root=document) => Array.from(root.querySelectorAll(sel));

      function createMiniPanel(title){
        const p = document.createElement("div");
        p.className = "miniPanel";
        p.style.marginTop = "10px";
        const tr = document.createElement("div");
        tr.className = "miniTitleRow";
      ...
      // Build pages by moving existing UI blocks into sheets
      // 1) CONTROL: use mobileBar action row(s)
      const mobileBar = document.getElementById("mobileBar");
      const ctrlWrap = createMiniPanel("조작");
      const statusWrapTop = createMiniPanel("상태");
      const shopWrap = createMiniPanel("구매");

      // Action row: wave/repair/aegis/sell
      const mbWave = document.getElementById("mbWave");
      if(mbWave){
        const row = mbWave.closest(".mbRow");
        safeMove(row, ctrlWrap);
      }
      // Energy row
      const mbEnergy = document.getElementById("mbEnergy");
  const mbBarrier = document.getElementById("mbBarrier");
      if(mbEnergy){
        const row = mbEnergy.closest(".mbRow");
        safeMove(row, ctrlWrap);
      }


// Restart button for mobile (Control tab)
const restartRow = document.createElement("div");
restartRow.className = "mbRow";
const restartBtn = document.createElement("button");
restartBtn.className = "mbBtn danger";
restartBtn.type = "button";
restartBtn.innerHTML = "<span>재시작</span><small>R</small>";
restartBtn.addEventListener("click", ()=>{ try{ ensureAudio(); }catch(e){} try{ SFX.play('click'); }catch(e){} restart(); }, {passive:true});
restartRow.appendChild(restartBtn);
ctrlWrap.appendChild(restartRow);

      // Turret install rows -> CONTROL (조작)
      const turWrap = createMiniPanel("포탑 설치");
      if(mobileBar){
        qsa(".mbRow.mbTurrets", mobileBar).forEach(r => safeMove(r, turWrap));
      }
      // Final support row -> CONTROL (조작)
      let finWrap = null;
      const mbFinalRow = document.getElementById("mbFinalRow");
      if(mbFinalRow){
        finWrap = createMiniPanel("최종 지원");
        safeMove(mbFinalRow, finWrap);
      }
      // Put turret panel above action buttons
      sheetCtrl.appendChild(turWrap);

      // Put control wrap into ctrl page
      sheetCtrl.appendChild(ctrlWrap);
      if(finWrap) sheetCtrl.appendChild(finWrap);

      // 2) STATUS: wire + turret selection + passive + preview + settings
      // Wire panel (canvas + text)
      const wirePanel = document.getElementById("wirePanel");
      if(wirePanel){
        // rename title if needed
        const t = wirePanel.querySelector(".miniTitle");
        if(t) t.textContent = "와이어 상태(Star UI)";
        sheetStatus.appendChild(wirePanel);
      }
      // Map gimmick panel (맵 기믹)
      const uiMapEl = document.getElementById("uiMap");
      if (uiMapEl){
        const mapPanel = uiMapEl.closest(".miniPanel");
        if (mapPanel) sheetStatus.appendChild(mapPanel);
      }
  ...
  const uiMapBonus = document.getElementById("uiMapBonus");
      if(uiPreview){
        const previewPanel = uiPreview.closest(".miniPanel");
        if(previewPanel) sheetStatus.appendChild(previewPanel);
      }
      // Mission panel (웨이브 목표)
      const uiMission = document.getElementById("uiMission");
      if (uiMission){
        const missionPanel = uiMission.closest(".miniPanel");
        if (missionPanel) sheetStatus.appendChild(missionPanel);
      }


      // Risk contract panel
      const uiContractHint = document.getElementById("uiContractHint");
      if(uiContractHint){
        const contractPanel = uiContractHint.closest(".miniPanel");
        if(contractPanel) sheetStatus.appendChild(contractPanel);
      }

      // Core passive pick panel
      const coreBtn = document.getElementById("btnCoreRebuild") || document.getElementById("btnCoreResonance");
      if(coreBtn){
        const corePanel = coreBtn.closest(".miniPanel");
        if(corePanel) sheetStatus.appendChild(corePanel);
      }

      // Settings chunk (sound/volume/bg/difficulty)
      const settingsPanel = createMiniPanel("설정");
      const idsForRows = ["btnSound","btnVol","volSlider","btnBg","btnDiffEasy","btnDiffNormal","btnDiffHard", "btnBugReport","btnSelfCheck"];
      const moved = new Set();
      idsForRows.forEach(id=>{
        const el = document.getElementById(id);
        if(!el) return;
        let row = el.closest(".row") || el.closest("#diffRow") || el.closest(".miniPanel");
        if(row && !moved.has(row)){
          moved.add(row);
          safeMove(row, settingsPanel);
        }
      });

      // --- Missing mobile settings restore: Auto-start + Speed ---
      // Auto-start next wave (checkbox + delay slider)
      const chkAutoStart = document.getElementById("chkAutoStart");
      if (chkAutoStart){
        const r = chkAutoStart.closest(".row");
        if (r && !moved.has(r)){
          moved.add(r);
          try{ r.style.flexWrap = "wrap"; r.style.gap = "8px"; }catch(e){}
          safeMove(r, settingsPanel);
        }
      }

      // Speed button (cycles speed)
      const btnSpeed = document.getElementById("btnSpeed");
      if (btnSpeed){
        const r = document.createElement("div");
        r.className = "row";
        r.appendChild(btnSpeed);
        safeMove(r, settingsPanel);
      }
      // Add speed/cheat row (from panel) or cheat row (from mobile bar)
      const mbCheatRow = document.getElementById("mbCheatRow");
      if(mbCheatRow){
        const cheatPanel = createMiniPanel("치트");
        safeMove(mbCheatRow, cheatPanel);
        sheetStatus.appendChild(cheatPanel);
      }
      // Add hint text if exists
      if(mobileBar){
        const hint = mobileBar.querySelector(".mbHint");
        if(hint){
          const hintWrap = document.createElement("div");
          hintWrap.className = "small";
          hintWrap.style.opacity = "0.85";
          hintWrap.style.marginTop = "10px";
          hintWrap.appendChild(hint);
          sheetStatus.appendChild(hintWrap);
        }
      }

      // Append wrappers
      sheetStatus.appendChild(settingsPanel);


      // Run records panel (런 기록)
      const uiRecords = document.getElementById("uiRecords");
      if (uiRecords){
        const recPanel = uiRecords.closest(".miniPanel");
        if (recPanel) sheetStatus.appendChild(recPanel);
      }
      // 3) SHOP: 소비 아이템 + 업그레이드
      const shopCons = document.getElementById("shopConsumables");
      if (shopCons) sheetShop.appendChild(shopCons);
      const upgPanelSide = document.getElementById("upgPanelSide");
      if(upgPanelSide){
        sheetShop.appendChild(upgPanelSide);
      }else{
        sheetShop.appendChild(shopWrap);
      }

      // Hide legacy UI that caused scroll; keep in DOM but not visible
      if(mobileBar) mobileBar.classList.add("hidden");

      // Tab switching
      const tabButtons = qsa(".tabBtn", tabs);
      const pages = { ctrl: sheetCtrl, status: sheetStatus, shop: sheetShop };
      let active = null;
      ...
      tabButtons.forEach(btn=>{
        btn.addEventListener("click", ()=>{
          ...
          if(!tab) return;
          if(sheet.classList.contains("open") && active === tab) close();
          else open(tab);
        }, {passive:true});
      });
