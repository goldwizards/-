...
function tryPickPassiveAt(mx, my){
  ...
  if (ptInRect(mx, my, L.confirm)) {
    const id = state.core.passivePreviewId || 'rebuild';
...
function drawPassivePickOverlay(){
  ...
  // 기본 미리보기(처음 표시 시)
  if (!state.core.passivePreviewId) state.core.passivePreviewId = 'rebuild';

  const L = passivePickLayout();
  const P = L.panel;
  const pick = state.core.passivePreviewId;
  const dPick = CORE_PASSIVES[pick];
  ...
  ctx.fillText('패시브 효과', desc.x + 12, desc.y + 10);
  ...
  ctx.strokeStyle = passiveAccent(pick);
  ctx.lineWidth = 2.6;
...
ctrlWrap.appendChild(restartRow);
      ...
      const idsForRows = ["btnSound","btnVol","volSlider","btnBg","btnDiffEasy","btnDiffNormal","btnDiffHard"];
      const moved = new Set();
