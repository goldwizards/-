...
// ---------- Spawning ----------
  function spawnEnemy(spec, idx){
    ...
    const hp = spec.hp * arch.hpMul * eliteHpMul * (diff.hpMul||1);
    const spd = spec.spd * arch.spdMul * eliteSpdMul * (0.92 + 0.16*Math.random()) * (diff.spdMul||1);
...
// 최종보스: 포탑 내성 (버스트 중 25% 부분 무시)
    ...
    if (isTurretCrit) {
      ...
      if (gt - (state._turretCritFxAt||0) > 0.10) {
        ...
        fxRing(p.x,p.y, 18, 190, "#fbbf24");
        fxRing(p.x,p.y, 28, 280, "rgba(253,230,138,0.95)");
        fxShardsBurst(p.x,p.y, "#fde68a", 8, 130, 0.60, 12);
        fxSparksBurst(p.x,p.y, "#fbbf24", 10, 150, 0.25, 3.0, 26);
      }
    } else if (p.splash > 0) {
      for (const e of state.enemies) {
...
state.enemies.length = 0;
    ...
    state.resFlashX = 0;
    state.resFlashY = 0;
    state.upg = { coreHp:0, coreShield:0, hpArmor:0, shieldArmor:0, shieldRegen:0, energyCannon:0, repair:0, turretDmg:0, turretFire:0, turretRange:0, slowPower:0, splashRadius:0, projSpeed:0, turretCrit:0, slowDuration:0, sellRefund:0, aegisTune:0, waveShield:0, shredFocus:0, breakerMark:0 };
    applyUpgrades();
...
// Wave button label + state
  ...
  btnWave.textContent = left > 0.05 ? `최종전 준비 (${left.toFixed(1)}s)` : "최종전 시작";
    ...
    if (uiPreview) {
      ...
      if (w >= 7) list.push(ENEMY_ARCH.supporter.name);
